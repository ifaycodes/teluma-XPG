...
export default function VaultPage() {
  ...
  function handleFileSelect(files: File[]) {
    ...
    if (oversized.length > 0) {
      alert(`${oversized.length > 1 ? 'These files are' : 'This file is'} too large. Maximum size is 15MB.\n${oversized.map(f => f.name).join('\n')}`)
      files = files.filter(f => f.size <= 15 * 1024 * 1024)
    }
    if (files.length === 0) return

    setPendingFiles(files)
    if (files.length === 1) {
      setUploadForm({ ...uploadForm, name: files[0].name.replace(/\.[^/.]+$/, '') })
    }
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    for (const file of pendingFiles) {
      try {
        ...
        formData.append('name', pendingFiles.length === 1 ? uploadForm.name : file.name.replace(/\.[^/.]+$/, ''))
    ...
    if (failed === 0) {
      showToast(succeeded === 1 ? 'Document uploaded successfully.' : `${succeeded} documents uploaded successfully.`)
    } else {
      showToast(`${succeeded} uploaded, ${failed} failed — check the file size and format and try again.`)
    }
  }
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
        ...
        <input
          ...
          multiple
          className="hidden"
          onChange={(e) => e.target.files && e.target.files.length > 0 && handleFileSelect(Array.from(e.target.files))}
        />
      </div>
          ...
          <div className="bg-[#FDFAF4] rounded-xl w-full max-w-md shadow-[0_8px_32px_rgba(28,28,28,0.18)]">
            <div className="flex items-center justify-between p-6 border-b border-[#1C1C1C]/10">
              <h3 className="text-lg font-bold text-[#1C1C1C]">
                {pendingFiles.length === 1 ? 'Upload Document' : `Upload ${pendingFiles.length} Documents`}
              </h3>
              <button
            ...
            <form onSubmit={handleUpload}>
              <div className="p-6 space-y-4">
                {pendingFiles.length === 1 ? (
                  <div className="flex items-center gap-3 bg-[#F5F0E8] rounded-lg p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#1C1C1C] truncate">{pendingFiles[0].name}</p>
                      <p className="text-xs text-[#2C1A0E]/50">{formatSize(pendingFiles[0].size)}</p>
                    </div>
                  </div>
                ) : (
                  <div className="max-h-40 overflow-y-auto space-y-2">
                    {pendingFiles.map((f, i) => (
                      <div key={i} className="flex items-center gap-3 bg-[#F5F0E8] rounded-lg p-3">
                        <span className="material-symbols-outlined text-[#A8192E] text-lg">attach_file</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-[#1C1C1C] truncate">{f.name}</p>
                          <p className="text-xs text-[#2C1A0E]/50">{formatSize(f.size)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-[#2C1A0E]/70 mb-1">
                    Tag {pendingFiles.length > 1 && <span className="font-normal text-[#2C1A0E]/50">(applied to all)</span>}
                  </label>
                  <select
              ...
              <div className="p-6 border-t border-[#1C1C1C]/10 flex flex-col gap-3">
                ...
                >
                  {uploading ? 'Uploading...' : pendingFiles.length === 1 ? 'Upload' : `Upload ${pendingFiles.length} Files`}
                </button>
                <button
                  type="button"
                  onClick={() => { setShowUploadModal(false); setPendingFiles([]) }}
